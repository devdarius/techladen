...
// Extract variant info from SKU attributes
function extractVariants(skuList: Array<{ sku_attr?: string }>) {
  ...
  for (const sku of skuList) {
    ...
    for (const part of parts) {
      ...
      if (match) {
        ...
        // Heuristic: short values are likely colors
        if (val.length < 30) {
          colors.add(val);
        }
      }
...
// Map category ID to German category name
function mapCategory(categoryId: number): string {
  const map: Record<number, string> = {
    200010248: 'Hüllen',
    200010249: 'Hüllen',
    200010250: 'Ladegeräte',
    200010251: 'Kabel',
    200010252: 'Schutzglas',
    200010253: 'Powerbanks',
  };
...
export async function POST(request: Request) {
  try {
    ...
    const resp = data['aliexpress_ds_product_get_response'] as
      | Record<string, unknown>
      | undefined;

    if (!resp) {
      return NextResponse.json(
        { error: 'Produkt nicht gefunden oder API-Fehler', raw: data },
        { status: 404 }
      );
    }
    ...
    const skuList = (resp.sku_list as Array<{ sku_attr?: string }>) ?? [];
    const variants = extractVariants(skuList);
    ...
    const product: Omit<Product, 'id'> = {
      slug,
      title,
      description: (resp.product_description as string) ?? '',
      category,
    ...
    const docRef = db.collection('products/de/items').doc(slug);
