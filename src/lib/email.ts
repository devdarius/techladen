...
const css = `
  ...
  .btn-dark{display:inline-block;background:#1A1A2E;color:#fff !important;text-decoration:none;padding:14px 32px;border-radius:8px;font-weight:700;font-size:15px;margin:20px 0;}
  ...
  return items.map(item => `
    <div class="product-row">
      ...
      <div style="flex:1;min-width:0;">
        ...
        <div class="product-qty">${[item.selectedColor, item.selectedModel].filter(Boolean).join(' · ')} × ${item.quantity}</div>
      </div>
...
function totalsHtml(order: Order) {
  return `
    <div class="total-row"><span style="color:#6C757D;">Zwischensumme</span><span>${order.subtotal.toFixed(2).replace('.', ',')} €</span></div>
    <div class="total-row"><span style="color:#6C757D;">Versand</span><span style="color:#2ECC71;font-weight:600;">${order.shipping === 0 ? 'Kostenlos' : order.shipping.toFixed(2).replace('.', ',') + ' €'}</span></div>
    <div class="total-row grand"><span>Gesamt (inkl. 19% MwSt.)</span><span class="amount">${order.total.toFixed(2).replace('.', ',')} €</span></div>
  `;
}
...
export async function sendVerificationEmail({
  ...
  const html = baseLayout(`
    ...
    <div class="body">
      <p style="font-size:16px;font-weight:600;margin:0 0 8px;">Hallo ${firstName}! 👋</p>
      <p style="color:#6C757D;font-size:14px;margin:0 0 28px;line-height:1.6;">
        Willkommen bei TechLaden.de! Um dein Konto zu aktivieren und loszulegen,
        bestätige bitte deine E-Mail-Adresse mit einem Klick auf den Button unten.
      </p>

      <div style="text-align:center;margin:32px 0;">
        <a href="${verifyUrl}" class="btn">✉️ E-Mail-Adresse bestätigen</a>
      </div>

      <div style="background:#F8F9FA;border-radius:8px;padding:16px 20px;margin-top:8px;">
        <p style="margin:0;font-size:12px;color:#6C757D;">
          Oder kopiere diesen Link in deinen Browser:<br/>
          <a href="${verifyUrl}" style="color:#E63946;word-break:break-all;">${verifyUrl}</a>
        </p>
      </div>

      <p style="font-size:12px;color:#6C757D;margin-top:20px;">
        ⏰ Dieser Link ist <strong>24 Stunden</strong> gültig.<br/>
        Falls du kein Konto bei TechLaden.de erstellt hast, kannst du diese E-Mail ignorieren.
      </p>
    </div>
  `, `Bestätige deine E-Mail-Adresse bei TechLaden.de`);

  return resend.emails.send({
    from: FROM,
    to: email,
    subject: '✉️ Bitte bestätige deine E-Mail-Adresse | TechLaden.de',
    html,
  });
}

// ─── Email: Zahlungsaufforderung ──────────────────────────────
export async function sendPaymentRequestEmail(
  ...
  const html = baseLayout(`
    ...
    <div class="body">
      ...
      <div class="info-box">${itemsHtml(order.items)}</div>
      <div style="padding:0 4px;">${totalsHtml(order)}</div>
      <div style="text-align:center;margin:28px 0;">
        <a href="${stripeUrl}" class="btn">💳 Jetzt bezahlen ${order.total.toFixed(2).replace('.', ',')} €</a>
        <p style="font-size:12px;color:#6C757D;margin:8px 0 0;">Der Zahlungslink ist 24 Stunden gültig.</p>
      </div>
...
// ─── Email: Bestellbestätigung + Rechnung ─────────────────────
export async function sendOrderConfirmationEmail(order: Order & { id: string }) {
  const orderNum = order.id.slice(-8).toUpperCase();
  const date = new Date(order.createdAt).toLocaleDateString('de-DE', { day: '2-digit', month: 'long', year: 'numeric' });
  ...
  const html = baseLayout(`
    ...
    <div class="body">
      <p style="font-size:16px;font-weight:600;margin:0 0 4px;">Hallo ${order.shippingAddress.firstName},</p>
      <p style="color:#6C757D;margin:0 0 8px;font-size:14px;">deine Zahlung wurde erfolgreich verarbeitet. Vielen Dank!</p>
      <p style="margin:0 0 24px;"><span class="badge">✓ Bezahlt</span></p>
      ...
      <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:14px 18px;margin-top:20px;">
        <p style="margin:0;font-size:13px;color:#92400e;">🚚 <strong>Lieferzeit: 3–7 Werktage.</strong> Du erhältst eine separate E-Mail mit der Sendungsverfolgung.</p>
      </div>
      <div style="text-align:center;margin-top:28px;">
        <a href="${BASE_URL}/mein-konto/bestellungen" class="btn-dark">Meine Bestellungen ansehen</a>
      </div>
    </div>
...
export async function sendShippingEmail(order: Order & { id: string }, trackingNumber?: string) {
  ...
  const html = baseLayout(`
    ...
    <div class="body">
      <p style="font-size:16px;font-weight:600;margin:0 0 4px;">Hallo ${order.shippingAddress.firstName},</p>
      <p style="color:#6C757D;margin:0 0 24px;font-size:14px;">dein Paket ist auf dem Weg zu dir! 🎉</p>
      ${trackingNumber ? `
      ...
      <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:14px 18px;margin-bottom:20px;">
        <p style="margin:0;font-size:13px;color:#1e40af;">📦 Geschätzte Lieferzeit: <strong>3–7 Werktage</strong></p>
      </div>
